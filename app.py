# CREATED BY JOACHIM ELIJAH COBAR
...
def exception_handler(errmsg="", e=""):
    ...
    if type(e).__name__ == "KeyboardInterrupt":
        end_time = (
            str(round(((time.time() - start_time) / float(60)), 2)) + " minutes"
            if (time.time() - start_time > 60.0)
            else str(round(time.time() - start_time)) + " seconds"
        )
        ...
        logger.debug(
            f"Golden Goose Ventures BOT Failed. | Time Taken = {end_time} {datetime.now().strftime('%m-%d-%Y %I:%M:%S %p')}"
        )
...
def start_spiders():
    ...
    """
    if environ.get(
        "GET_SPIDERLIST_FROM_DASHBOARD", GGV_SETTINGS.GET_SPIDERLIST_FROM_DASHBOARD
    ):
    ...
    for spider in spider_list:
        ...
        save_counter += 1
        logger.debug(
            f"Save Progress Counter {save_counter}/{GGV_SETTINGS.DB_SAVE_SPIDER_COUNTER}"
        )
        logger.info(f"Removing {spider} from Pending Spider List...")
        save_spider_list.remove(spider)
        if (
            environ.get("DEPLOYED")
            and not environ.get("SCHEDULE_EMAIL_COPY")
            and environ.get("SEND_EMAIL_OFFLINE_COPY")
            and not len(save_spider_list)
        ):
            send_email()
        if (
            save_counter >= GGV_SETTINGS.DB_SAVE_SPIDER_COUNTER
        ) and GGV_SETTINGS.SAVE_DROPBOX_LIST:
...
def ggventures_bot_worker():
    ...
    try:
        ...
        logger.debug(
            f"Golden Goose Ventures BOT Finished successfully. | Time Taken = {end_time} {datetime.now().strftime('%m-%d-%Y %I:%M:%S %p')}"
        )
